...
def main():
    ...
    for itr in range(number_of_iterations):
        ...
        for attempt in range(number_of_attempts):
            ...
            print(f"{attempt}:AABL: {TP}, Adjusted: {TP_my}, MostCommonGuesser: {TP_mostcommon}")
        ...
        ax.plot(range(number_of_attempts), np.mean(all_TPs_mostcommon[:itr + 1], axis=0) / (np.array(range(number_of_attempts)) + 1), 'g--', label="MCG")
